...
used_features  = { "none"     : { "F0"   : [ "mean", "mean_tile", "median", "median_tile", "mode", "mode_tile", "binsize_5", "bin_0_3", "bin_1_3", "bin_2_3" ],
		   ...
		   "quantile" : { "F0"   : [ "mean", "mean_tile", "median", "median_tile", "mode", "mode_tile", "binsize_5" ] },
		   "uniform"  : { "F0"   : [ "mean", "mean_tile", "median", "median_tile", "mode", "mode_tile", "binsize_5" ] } }
...
for i in range ( n_folds ) :
	...
	if os.path.isfile ( model_fname ) :
		...
		if i + 1 > 7 :
			...
			model = RandomForestClassifier ( n_estimators = n_estimators, criterion = criterion, max_depth = max_depth, n_jobs = 4, random_state = rs + 17 * ( i + 19 ) )

		elif i + 1 > 5 :

			model = RandomForestClassifier ( n_estimators = n_estimators, criterion = criterion, max_depth = max_depth, n_jobs = 2, random_state = rs + 17 * ( i + 19 ) )
