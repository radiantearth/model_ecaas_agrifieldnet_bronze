...
hist_fname     = os.path.join ( "data", "d" + base_name + "-hist.csv"   )
...
print ( f"All json files : { len ( all_json_files ) }" )
print ( f"All tiff files : { len ( all_tiff_files ) }" )
print ( f"B01 tiff files : { len ( b01_tiff_files ) }" )
...
for i, I in enumerate ( b01_tiff_files ) :
	...
	if tile_id not in tiles_done :
		...
		path       = I [ : I.rindex ( "/" ) + 1 ]
		tifs       = []
		train_ids  = os.path.join ( input_dir, "ref_agrifieldnet_competition_v1_labels_test/ref_agrifieldnet_competition_v1_labels_test_"   + tile_id + "/field_ids.tif" )
		test_ids   = os.path.join ( input_dir, "ref_agrifieldnet_competition_v1_labels_train/ref_agrifieldnet_competition_v1_labels_train_" + tile_id + "/field_ids.tif" )
		...
		# Fields in this tile
		print ( "\tFields" )
		for j, J in enumerate ( [ train_ids, test_ids ] ) :
