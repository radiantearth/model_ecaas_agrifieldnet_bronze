# Same as m013-37 but saves
# cluster models to make
# inference easier later on
import os
import pickle
...
if os.path.isfile ( clusters_fname ) :
	...
	for i, j in zip ( [ AffinityPropagation ( max_iter = 1000, random_state = rs ),
		...
		for k in [ "small", "medium", "large", "huge" ] :
			...
			print ( "\t\tsaving" )
			dump ( i, os.path.join ( "models", f"{ model_basename }-{ j }-{ k }-selected.joblib" ) )
			...
			print ( "\t\tsaving" )
			dump ( i, os.path.join ( "models", f"{ model_basename }-{ j }-{ k }-cluster.joblib" ) )
