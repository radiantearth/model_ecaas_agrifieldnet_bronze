...
used_features  = { "none"     : { "F0"   : [ "mean", "mean_tile", "binsize_5", "bin_0_3", "bin_1_3", "bin_2_3" ],
				  "F+3"  : [ "mean", "mean_tile", "binsize_5" ],
				  "F+20" : [ "mean", "binsize_7", "bin_0_7", "bin_1_7", "bin_2_7", "bin_3_7", "bin_4_7", "bin_5_7", "bin_6_7" ],
				  "F+40" : [ "mean", "binsize_7", "bin_0_7", "bin_1_7", "bin_2_7", "bin_3_7", "bin_4_7", "bin_5_7", "bin_6_7" ] },
		   "quantile" : { "F0"   : [ "diff", "mean_tile", "binsize_5" ] },
		   "uniform"  : { "F0"   : [ "diff", "mean_tile", "binsize_5" ] } }

# Cluster features
small_features = { "none"     : { "F0"   : [ "mean", "binsize_3" ],
 				  "F+3"  : [ "mean", "binsize_3" ],
				  "F+20" : [ "mean", "binsize_5", "bin_0_3", "bin_1_3", "bin_2_3" ] },
		   "quantile" : { "F0"   : [ "diff" ] },
		   "uniform"  : { "F0"   : [ "diff" ] } }
medm_features  = { "none"     : { "F0"   : [ "mean", "binsize_5" ],
 				  "F+3"  : [ "mean", "binsize_5" ],
				  "F+20" : [ "mean", "binsize_7", "bin_0_5", "bin_1_5", "bin_2_5", "bin_3_5", "bin_4_5" ] },
		   "quantile" : { "F0"   : [ "diff" ] },
		   "uniform"  : { "F0"   : [ "diff" ] } }
large_features = { "none"     : { "F0"   : [ "mean", "binsize_5", "bin_0_3", "bin_1_3", "bin_2_3" ],
 				  "F+3"  : [ "mean", "binsize_5", "bin_0_3", "bin_1_3", "bin_2_3" ],
				  "F+20" : [ "mean", "binsize_7", "bin_0_5", "bin_1_5", "bin_2_5", "bin_3_5", "bin_4_5" ] },
		   "quantile" : { "F0"   : [ "diff" ] },
		   "uniform"  : { "F0"   : [ "diff" ] } }
huge_features  = { "none"     : { "F0"   : [ "mean", "binsize_7", "bin_0_5", "bin_1_5", "bin_2_5", "bin_3_5", "bin_4_5" ],
				  "F+3"  : [ "mean", "binsize_7", "bin_0_5", "bin_1_5", "bin_2_5", "bin_3_5", "bin_4_5" ],
				  "F+20" : [ "mean", "binsize_7", "bin_0_7", "bin_1_7", "bin_2_7", "bin_3_7", "bin_4_7", "bin_5_7", "bin_6_7" ] },
		   "quantile" : { "F0"   : [ "diff", "binsize_5" ] },
		   "uniform"  : { "F0"   : [ "diff", "binsize_5" ] } }
			...
			for k in d [ i ] :
				...
				for l in value_calc_cols :
					...
					if l in d [ i ][ k ] :
						...
						if l == "mean" and D == "data" :

							x_cols_ld.append ( len ( cols ) )
...
cluster_labels = [ "affinity_propagation", "gmm_30", "gmm_15", "gmm_5", "kmeans_40", "kmeans_20", "kmeans_10", "optics" ]
cluster_index  = []
...
if os.path.isfile ( clusters_fname ) :
	...
	for i, j in zip ( [ AffinityPropagation ( max_iter = 1000, random_state = rs ),
			    GaussianMixture ( n_components = 30, covariance_type = "spherical", max_iter = 500, n_init = 10, init_params = "k-means++", random_state = rs + 101 ),
			    GaussianMixture ( n_components = 15, covariance_type = "spherical", max_iter = 500, n_init = 10, init_params = "k-means++", random_state = rs + 301 ),
			    GaussianMixture ( n_components =  5, covariance_type = "spherical", max_iter = 500, n_init = 10, init_params = "k-means++", random_state = rs + 601 ),
...
# Sample and add linear discriminant model
print ( "Sampling and adding linear discriminant model" )
...
for i in range ( n_folds ) :
	...
	kf        = StratifiedKFold ( n_splits = n_folds_ld )

	X_ld      = np.zeros ( [     X.shape [ 0 ], n_labels ], dtype = float )
	X_svm     = X_ld.copy ()
	valid_ld  = np.zeros ( [ valid.shape [ 0 ], n_labels ], dtype = float )
	valid_svm = valid_ld.copy ()

	for j, k in kf.split ( np.arange ( X.shape [ 0 ] ), y ) :

		print ( "\t\t\t\tld" )

		model      = LinearDiscriminantAnalysis ( solver = "eigen", covariance_estimator = OAS ( store_precision = False, assume_centered = False ) )

		model.fit ( X [ j ][ :, x_cols_ld ], y [ j ] )

		X_ld [ k ] = model.predict_proba ( X [ k ][ :, x_cols_ld ] )
		valid_ld  += model.predict_proba ( valid [ :, x_cols_ld ] ) / n_folds_ld

		if test_ld is None :

			test_ld  = model.predict_proba ( test [ :, x_cols_ld ] ) / ( n_folds * n_folds_ld )
		...
		model      = SVC ( probability = True, random_state = rs + j [ 0 ] + j [ 1 ] + j [ 2 ] * j [ 3 ] )

		model.fit ( X [ j ][ :, x_cols_ld ], y [ j ] )

		X_svm [ k ] = model.predict_proba ( X [ k ][ :, x_cols_ld ] )
		valid_svm  += model.predict_proba ( valid [ :, x_cols_ld ] ) / n_folds_ld

		if test_svm is None :

			test_svm  = model.predict_proba ( test [ :, x_cols_ld ] ) / ( n_folds * n_folds_ld )

		else :

			test_svm += model.predict_proba ( test [ :, x_cols_ld ] ) / ( n_folds * n_folds_ld )

	samples.append ( ( np.concatenate ( [     X,     X_ld,     X_svm ], axis = 1 ), y     ) )
	valids.append  ( ( np.concatenate ( [ valid, valid_ld, valid_svm ], axis = 1 ), labsv ) )

# Combine test with LD probabilities
test           = np.concatenate ( [ test, test_ld, test_svm ], axis = 1 )
